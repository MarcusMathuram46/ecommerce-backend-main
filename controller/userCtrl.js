...
const userCart = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (alreadyExistCart) {
      // Update existing cart instead of removing it
      alreadyExistCart.products = []; // Clear existing products
      ...
      for (let i = 0; i < alreadyExistCart.products.length; i++) {
        alreadyExistCart.cartTotal += alreadyExistCart.products[i].price * alreadyExistCart.products[i].count;
      }

      await alreadyExistCart.save(); // Save the updated cart
